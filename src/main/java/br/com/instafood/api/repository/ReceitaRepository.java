...
public interface ReceitaRepository extends CrudRepository<Receita, Integer> {
	...
	@Query("SELECT DISTINCT r.id, r.titulo, r.imagem FROM receita r \n"
			...
			+ "    AND t.id IN :tags;")
	Iterable<Receita> findByParams(List<Integer> ingredientes, List<Integer> utensilios, List<Integer> tags);
	Iterable<Receita> saveAll(List<Integer> ingredientes, List<Integer> utensilios, List<Integer> tags);
}
